  ...
  return (
    <main className="offers-page">
      <header className="offers-header">
        ...
        <Link className="text-link inline-flex items-center gap-1" href="/">
      ...
      <section className="offers-grid">
        {items.length ? (
          items.map((item) => (
            <Link className="public-offer-card" href={`/ofertas/${item.slug}`} key={item.id}>
              <div className="public-offer-image">
                <Image src={item.imageUrl} alt={item.title} fill sizes="(max-width: 700px) 100vw, 33vw" />
              </div>
